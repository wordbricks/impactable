...
| --- | --- | --- | --- |
| M1 | Define Velen client and result/error types | completed | `internal/gitimpact` contains `VelenClient`, `WhoAmIResult`, `OrgResult`, `Source`, `QueryResult`, and `VelenError` with JSON tags aligned to CLI payloads. |
...
## Current progress
- Added `internal/gitimpact/types.go` with Velen result/error types.
