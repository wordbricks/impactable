...
## Goal
Implement Step 5 for `git-impact` by wiring CLI arguments into a structured analysis context, building the agent initial prompt from that context and config, and adding a `check-sources` flow that validates required OneQuery source connectivity.
...
1. **Context construction and prompt assembly** — `completed`
   ...
   - Implement `BuildInitialPrompt(...)` to inject since/pr/feature inputs and configured OneQuery source keys.
...
## Key decisions
- Reuse existing `Config`, `AnalysisContext`, `Source`, OneQuery client, and engine types as-is.
...
- Verify provider-type fallback behavior against real-world `onequery source list` payload variations beyond test fixtures.
