...
func TestRun_ParseErrorStructuredInJSONMode(t *testing.T) {
	...
	code := Run([]string{"check-sources", "--pr", "1", "--output", "json"}, t.TempDir(), strings.NewReader(""), &stdout, &stderr)
	if code != 1 {
...
func TestRun_ParseErrorStructuredInNDJSONMode(t *testing.T) {
	...
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	code := Run([]string{"report-scaffold", "--mode", "pdf", "--output", "ndjson"}, t.TempDir(), strings.NewReader(""), &stdout, &stderr)
	if code != 1 {
		t.Fatalf("expected exit code 1, got %d", code)
	}
	if stderr.Len() != 0 {
